...
const NAV_ITEMS: NavItem[] = [
  { label: "AI Matches", href: "/talent/matches", icon: Sparkles },
  { label: "Browse Jobs", href: "/talent/jobs", icon: Search },
  { label: "My Profile", href: "/talent/profile", icon: User },
  { label: "Saved Jobs", href: "/talent/saved", icon: Bookmark },
  { label: "Applications", href: "/talent/applications", icon: ClipboardList },
  { label: "Courses", href: "/talent/courses", icon: BookOpen },
  { label: "Messages", href: "/talent/messages", icon: MessageSquare, badgeKey: "messages" },
  { label: "Notifications", href: "/talent/notifications", icon: Bell, badgeKey: "notifications" },
];

export default function TalentShell({ children }: { children: React.ReactNode }) {
      ...
      <div className="flex min-h-screen items-center justify-center bg-slate-50 px-6 text-center">
        <div>
          <p className="text-lg font-semibold text-wazifny-navy">
            This area is for talent accounts only.
          </p>
      ...
      <aside className="hidden w-72 shrink-0 flex-col bg-wazifny-navy px-5 py-6 md:flex">
        ...
        <div className="mb-6 flex items-center gap-3 rounded-xl bg-white/5 px-3 py-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-slate-400">Talent Account</p>
          </div>
        </div>
